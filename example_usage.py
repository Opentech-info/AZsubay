...
def main():
    ...
    responses = []
    responses.append(navigate_menu(session["session_id"], "1"))  # To Phone
    responses.append(navigate_menu(session["session_id"], "0722334455"))  # Phone number
    responses.append(navigate_menu(session["session_id"], "1500"))  # Amount
    responses.append(navigate_menu(session["session_id"], "1"))  # Confirm
    
    for i, resp in enumerate(responses, 1):
        print(f"   Step {i}: {resp['response'][:50]}...")
    ...
    from azsubay.utils.crypto import hash_data, generate_secure_token, validate_phone_number, format_amount
