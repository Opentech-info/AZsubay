...
def test_verify_identity_invalid_provider():
    ...
    with pytest.raises(
        ProviderError, match="Unsupported KYC provider: InvalidProvider"
    ):
...
def test_submit_documents_empty_document_data():
    ...
    with pytest.raises(
        DocumentError, match="Required field 'front_image' is missing or empty."
    ):
...
def test_submit_documents_invalid_base64():
    ...
    with pytest.raises(
        DocumentError, match="Field 'front_image' contains invalid base64 data."
    ):
...
def test_kyc_make_provider_request_api_error(requests_mock):
    ...
    requests_mock.post(
        "https://api.smileidentity.com/v1/verify",
        status_code=500,
...
def test_verify_identity_general_exception(monkeypatch):
    ...
    with pytest.raises(
        VerificationError, match="Identity verification failed: Simulated general error"
    ):
